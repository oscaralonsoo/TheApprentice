...
void PlayerMechanics::Update(float dt) {
    ...
    if (isInvulnerable) {
        if (invulnerabilityTimer.ReadSec() >= invulnerabilityDuration) {
            ...
            if (blinkTimer.ReadMSec() >= blinkInterval) {
                ...
                blinkTimer.Start();
                blinkInterval += 150.0f; // para que parpadee m�s lento con el tiempo
            }
        }
    }
    ...
    if (!isDashing) {
        ...
        if (!knockbackActive) { // solo permitimos movimiento si no hay empuj�n activo
            if (Engine::GetInstance().input->GetKey(SDL_SCANCODE_A) == KEY_REPEAT) {
                velocity.x = -speed;
            }
...
void PlayerMechanics::OnCollision(PhysBody* physA, PhysBody* physB) {
    ...
    case ColliderType::PUSHABLE_PLATFORM:
        ...
        if (!jumpCooldownActive) {
            ...
            // Tambi�n se comporta como una pared: cancela el dash si est� activo
            if (isDashing) {
                CancelDash();
            }
            knockbackActive = false;
...
void PlayerMechanics::HandleInput() {
    ...
    // Si no se est� pulsando izquierda ni derecha, y no est� saltando, cayendo, atacando o en wallslide...
...
void PlayerMechanics::HandleJump() {
    ...
    if (Engine::GetInstance().input->GetKey(SDL_SCANCODE_SPACE) == KEY_DOWN) {
        if (isOnGround || (doubleJumpUnlocked && jumpCount < maxJumpCount)) {
            velocity.y = -jumpForce; // sin el 0.8f, para dar un impulso inicial m�s fuerte
            isJumping = true;
            isHoldingJump = true;
    ...
    // Ca�da r�pida autom�tica al soltar salto (estilo Hollow Knight)
